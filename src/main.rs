use clap::Parser;
use hn_decypher::decypher_str;
use inner::{HnDecypherError, MainArgs};
...
///
/// This only adds value with the `save_read` feature enabled but is included unconditionally so
/// that main can be essentially the same with or without the feature.
enum OutputBehavior {
    /// Output the file content verbatim and do not print any headers
    ///
    /// Only used when save_read is enabled
    #[cfg(feature = "save_read")]
    Verbatim,
...
/// Arguments that we want to keep common between compiling with/without the save_read feature, and
/// between multiple subcommands when save_read is enabled.
///
/// This struct exists for two reasons:
/// 1. We want main to be the same with or without the save_read feature
/// 2. When save_read is enabled, this helps us get the command format we want.
///
/// Elaborating on point 2: we want the command to be `hn-decypher [mode] [arguments]`, but to accomplish that,
/// arguments have to be descendants of each mode. If the arguments were children of MainArgs directly, the command
/// format would instead be `hn-decypher [some arguments] [mode] [other arguments]` which we don't want.
///
/// This struct abstracts some of that away, so that main can refer to these in a common way, but
...
#[cfg(not(feature = "save_read"))]
mod inner {
    use super::{CommonArgs, OutputBehavior};
    use clap::Parser;
    use hn_decypher::DecypherError;
    use std::{fs, io, path::PathBuf};
    use thiserror::Error;

    #[derive(Error, Debug)]
    pub enum HnDecypherError {
    ...
    impl MainArgs {
        pub fn get_input(&self) -> Result<String, HnDecypherError> {
            fs::read_to_string(&self.file_path).map_err(HnDecypherError::FileRead)
        }

        pub fn into_common(self) -> CommonArgs {
            let behavior = if self.quiet {
